const {
  ...
  updateFactura,
  deleteFactura
} = require('../Services/Servicios_Facturas');
...
const crearFactura = async (req, res) => {
  try {
    ...
    if (error.status === 400) {
      return res.status(status).json({ message: error.message || 'El juego no dispone de stock', error: error.message });
    }
    else {
    return res.status(status).json({ message: error.message || 'Error al crear la factura', error: error.message });
  }
  }
};
...
const actualizarFactura = async (req, res) => {
  try {
    ...
    // Valida si los campos obligatorios si se envían
    ...
    // Valida que los campos de juegos no estén vacíos si se envían
    const juegosData = juegos || lista_de_juegos || listaDeJuegos;
